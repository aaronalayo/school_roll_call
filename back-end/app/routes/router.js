...
import deleteOldRows from "../../scheduler.js";
	...
	if (userId === "" || subjectId === "" || expirationTime.length === "") {
		...
		if (user.length == 0) {
			...
			if (role[0].role === "TEACHER") {
				let generatedInfo;
				const code = await Code.query().select().where({ subject_uuid: subjectId });
				if (code.length > 0) {
					const subject = await Subject.query().select().where({ subject_uuid: subjectId });
					let expTime = moment(moment(code[0].expires_at) - moment()).minutes();
					if (expTime >= 1) {
						generatedInfo = {
							subjectName: subject[0].subject_name,
							expirationTime: expTime,
							code: Number(code[0].code)
						};
						
					} else {
						await Code.query().delete().where({subject_uuid: subjectId});
						generatedInfo = await getGenerateInfo(subjectId, user[0].user_uuid, expirationTime);
